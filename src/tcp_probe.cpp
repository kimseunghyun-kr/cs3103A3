...
static in_addr pick_ipv4(const std::vector<ResolvedAddress>& addrs) {
    for (const auto& ra : addrs) if (ra.family == AF_INET)
...
TcpProbe::trace(const std::string& host, int port, int max_hops, int timeout_ms,
    ...
    // Receive sockets
    IcmpListener icmp; if (!icmp.open()) throw std::runtime_error("Need CAP_NET_RAW/root for ICMP");
    int tcp_recv_sock = ::socket(AF_INET, SOCK_RAW, IPPROTO_TCP);
    if (tcp_recv_sock < 0) throw std::runtime_error("Need CAP_NET_RAW/root to sniff TCP");

    // Optional raw sender
    int raw_send_sock = -1;
    if (mode == SendMode::Raw) {
        raw_send_sock = ::socket(AF_INET, SOCK_RAW, IPPROTO_TCP);
        if (raw_send_sock < 0) throw std::runtime_error("raw send socket failed");
        int on = 1; (void)setsockopt(raw_send_sock, IPPROTO_IP, IP_HDRINCL, &on, sizeof(on));
    }
    ...
    for (int ttl = 1; ttl <= max_hops && !destination_reached; ++ttl) {
        in_flight.clear(); HopAgg agg{};
        if (diag) diag->log("HOP " + std::to_string(ttl) + ": send 3 probes");

        // ----- SEND 3 PROBES for this TTL -----
        struct ProbeSock { int fd; };
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (mode == SendMode::Raw) {
                ...
                ip->frag_off=0; ip->ttl=static_cast<uint8_t>(ttl);
                ip->protocol=IPPROTO_TCP; ip->saddr=src_ip.s_addr; ip->daddr=dst_ip.s_addr;
                ip->check=0;

                std::memset(tcp,0,sizeof(tcphdr));
                tcp->source=htons(sport); tcp->dest=htons(static_cast<uint16_t>(port));
                tcp->seq=htonl((ttl<<24) | (i<<16) | 0x1234); tcp->doff=5; tcp->syn=1; tcp->window=htons(65535);

                in_flight[sport] = ProbeState{ttl, clk::now(), false};
                (void)::sendto(raw_send_sock, pkt.data(), pkt.size(), 0,
                               reinterpret_cast<const sockaddr*>(&dst), sizeof(dst));

                if (diag) diag->log("PROBE_SENT mode=raw ttl=" + std::to_string(ttl) +
                                    " idx=" + std::to_string(i) + " sport=" + std::to_string(sport));
            } else { // Auto / Connect
                int s = ::socket(AF_INET, SOCK_STREAM, 0);
                if (s < 0) { if (diag) diag->log("WARN socket() failed"); continue; }

                sockaddr_in src{}; src.sin_family = AF_INET; src.sin_port = htons(sport); src.sin_addr = src_ip;
                (void)::bind(s, reinterpret_cast<sockaddr*>(&src), sizeof(src));

                int ttl_val = ttl; (void)::setsockopt(s, IPPROTO_IP, IP_TTL, &ttl_val, sizeof(ttl_val));
                ...
                if (diag) diag->log("PROBE_SENT mode=connect ttl=" + std::to_string(ttl) +
                                    " idx=" + std::to_string(i) + " sport=" + std::to_string(sport));
            }
        }
        ...
        while (clk::now() < deadline && replies_seen < 3) {
            ...
            int maxfd = -1;
            FD_SET(icmp.fd(), &rfds); maxfd = std::max(maxfd, icmp.fd());
            ...
            if (FD_ISSET(icmp.fd(), &rfds)) {
                if (auto te = icmp.recv_time_exceeded()) {
                    ...
                    if (it != in_flight.end() && !it->second.done) {
                        ...
                        if (agg.count == 0) agg.ip = te->from_ip;
                        agg.count++;
                        if (agg.count == 1) { agg.min_ms = agg.max_ms = rtt; }
                        else { agg.min_ms = std::min(agg.min_ms, rtt); agg.max_ms = std::max(agg.max_ms, rtt); }
                        agg.sum_ms += rtt;
                        it->second.done = true; replies_seen++;

                        if (diag) diag->log("ICMP_TIME_EXCEEDED from=" + te->from_ip +
                                            " sport=" + std::to_string(te->orig_sport) +
                                            " inner_ttl=" + std::to_string(te->orig_ttl) +
                                            " rtt_ms=" + std::to_string(rtt));
                    } else {
                        if (diag) diag->log("ICMP_TIME_EXCEEDED (unmatched) sport=" + std::to_string(te->orig_sport));
                    }
                }
            }
            ...
            if (FD_ISSET(tcp_recv_sock, &rfds)) {
                ...
                if (n > 0) {
                    auto* ip = reinterpret_cast<iphdr*>(buf.data()); size_t off = ip->ihl*4;
                    if (off + sizeof(tcphdr) <= static_cast<size_t>(n)) {
                        ...
                        if (it != in_flight.end() && !it->second.done && ip->saddr == dst_ip.s_addr) {
                            bool synack = (tcp->syn && tcp->ack); bool rst = (tcp->rst != 0);
                            if (synack || rst) {
                                double rtt = std::chrono::duration<double, std::milli>(clk::now() - it->second.t0).count();
        ...
        for (auto &kv : probe_socks) { if (kv.second.fd >= 0) ::close(kv.second.fd); }
        ...
        if (diag) {
            ...
            if (agg.count == 0) diag->log("NO_ICMP_THIS_HOP ttl=" + std::to_string(ttl) + " (timeout)");
        }

        ProbeHopSummary row{}; row.ttl = ttl; row.reached = agg.reached; row.num_replies = agg.count;
        if (agg.count > 0) { row.hop_ip = agg.ip; row.rtt_min_ms = agg.min_ms; row.rtt_max_ms = agg.max_ms; row.rtt_avg_ms = agg.sum_ms / agg.count; }
        out.push_back(row);

        if (destination_reached) {
            if (diag) diag->log("STOP: destination reached at ttl=" + std::to_string(ttl));
            break;
        }
    }

    if (raw_send_sock >= 0) ::close(raw_send_sock);
    ::close(tcp_recv_sock); icmp.close();

    // Post-run heuristic: only gateway + destination responded → likely ICMP11 blocked / TTL mangled
    if (diag) {
        int responders = 0, first = -1, last = -1;
        for (auto &r : out) if (r.num_replies > 0) { responders++; if (first < 0) first = r.ttl; last = r.ttl; }
